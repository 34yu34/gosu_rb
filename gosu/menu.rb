...
class Menu
  def initialize(width, height, mouse_x, mouse_y)
    ...
    @line << Buton.new('Start', height / 8, width / 4, center(width, width / 4), @line.last.next_pos_y, Affichage::GAME)
    @line << Buton.new('Quit', height / 8, width / 4, center(width, width / 4), @line.last.next_pos_y, Affichage::QUIT)
  end

  def next_page(current_page)
    @line.each do |line|
      if line.instance_of? Buton
